        ...
        implements DirectoryLister
{
    ...
    private Configuration getRawConfiguration(Configuration conf)
    {
        if (conf.getClass().equals(Configuration.class)) {
            return conf;
        }
        if (conf instanceof WrapperJobConf) {
            conf = ((WrapperJobConf) conf).getConfig();
        }
        else if (conf instanceof CopyOnFirstWriteConfiguration) {
            conf = ((CopyOnFirstWriteConfiguration) conf).getConfig();
        }
        else {
            throw new RuntimeException(String.format("unexpected Configuration subType %s", conf.getClass().getName()));
        }
        return getRawConfiguration(conf);
    }
